...
<head>
  ...
  <title>AEMWIA | Register Members</title>
    ...
    <section class="content">
      ...
      <div class="row">
        <section class="col-lg-12">
          ...
          <div class="box box-info">
            <div class="box-header">
              ...
              <h3 class="box-title">Members found in Database</h3>
		...
		<table class="table">
			   ...
			   $sql = "SELECT * FROM user_info where role = 'member' ORDER BY full_name limit $page1,10";
             $result = $conn->query($sql);
		    ...
		    print '<td><a title="Edit '.$row["full_name"].'" class="btn btn-block btn-primary btn-xs" href="update_user_info.php?ref='.$row["user_id"].'"><i class="fa fa-edit"></i></a>
			<a '; ?> 	<a onclick="return confirm('Are you sure you want to delete <?php echo $row['full_name']; ?> ?');" <?php print 'title="Delete '.$row["full_name"].'" class="btn btn-block btn-danger btn-xs" href="delete_user_info.php?ref='.$row["user_id"].'&page='.$page1.'"><i class="fa fa-trash-o"></i></a>
        ...
        Registered member will be shown here
      </div>';
                  }
                 $conn->close();
			   ?>
			  ...
			  <?php
						 include '../includes/config.php';
						$sql = "SELECT * FROM user_info where role = 'member' ORDER BY full_name";
$result = $conn->query($sql);
...
for ($b=1;$b<=$a;$b++)
{
	?> <li class="paginate_button"><a href="members.php.php?page=<?php echo $b; ?>" ><?php echo $b. " "; ?></a></li><?php
}
}
$conn->close();
						?>
