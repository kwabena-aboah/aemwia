...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
       ...
       header("location:new_member.php?msg=Email $stdem is not available&member=$student");
    }
} else {
...
if ($conn->query($sql) === TRUE) {
    header("location:new_member.php?message=$stdname have been registered with ID $stdno");
} else {
	$error = $conn->error;
     header("location:new_member.php.php?err=$error");
}
